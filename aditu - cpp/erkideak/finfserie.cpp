...
void finfserie(const vector<double> &zentro, const vector<double> &xpuntu, const double k, const int nhmoz, vector<vector<complex<double> > > &serie, vector<vector<complex<double> > > &dserie)
{
...
if (fabs(cos(theta)-1)<1.e-6) /** Ipar-lurmuturra x ardatzeko esferikoak **/
{
...
else if (fabs(cos(theta)+1)<1.e-6) /** Hego-lurmuturra x ardatzeko esferikoak **/
{
    kos=-1.0;
    ksk=0.0;
    fi=0.0;
...
/** Normalizazio konstantea m=0, n=0 **/

for (m=0; m<=nhmoz; m++)
    {
    /** m+1 ordenako Hankel funtzioa **/
    hannorm.push_back(1.0);
    ...
    /** m ordenako Hankel funtzioaren deribatua **/
    ...
    for (n=1; n<=m; n++)
        ...
        dlp=-(1+m)*ksk*kos*lp+(1+m-n)*ksk*lpp;
